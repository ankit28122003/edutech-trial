...
import Container from '../../components/ui/Container';
import Button from '../../components/ui/Button';
import HeroSuccessCollage from './HeroSuccessCollage';
import { TRUST_LOGOS } from '../../data/trustLogos';
...
export default function Hero() {
  ...
  return (
    <section className="relative overflow-hidden bg-surface-alt pt-14 pb-16 sm:pt-20 sm:pb-20">
      <Container className="relative">
        <div className="grid grid-cols-1 items-center gap-14 lg:grid-cols-[1.05fr_0.95fr] lg:gap-8">
          <div>
            <motion.h1
              ...
              className="text-4xl font-semibold leading-[1.08] tracking-tight text-ink sm:text-5xl lg:text-[3.1rem]"
            >
            ...
            >
              ...
              <span className="font-semibold text-ink">new-age technologies</span> and we'll help you get{' '}
              <span className="font-semibold text-ink">job-ready</span>, fast.
            </motion.p>
            ...
            <motion.div
              ...
              className="mt-8 flex flex-wrap items-center gap-3"
            >
              <Button to="/courses" variant="accent" size="lg">
                Explore All Courses
              </Button>
              <Button to="/contact" variant="outline" size="lg">
                Corporate Training <ArrowRight size={17} />
              </Button>
            </motion.div>
            ...
            <motion.div
              ...
              className="mt-10"
            >
              <p className="text-sm font-medium text-ink-muted">Trusted by</p>
              <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-2">
                {TRUST_LOGOS.map((name) => (
                  <span key={name} className="text-sm font-semibold text-ink-soft">
                    {name}
                  </span>
                ))}
              </div>
              <p className="mt-2 text-sm text-ink-muted">and 4,500+ companies across the globe</p>
            </motion.div>
            ...
            >
              <div className="flex -space-x-3">
                {avatarSample.map((person) => (
                  <img
                    key={person.name}
                    src={person.avatar}
                    alt=""
                    loading="lazy"
                    className="h-9 w-9 rounded-full border-2 border-surface-alt object-cover"
                  />
                ))}
              </div>
              <div>
                <p className="text-sm font-semibold text-ink">Rated by Learners</p>
                <p className="flex items-center gap-1.5 text-sm text-ink-muted">
                  <Star size={14} className="fill-accent-500 text-accent-500" strokeWidth={0} />
