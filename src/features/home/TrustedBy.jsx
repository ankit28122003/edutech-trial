...
const COMPANY_LOGOS = [
  { name: 'TCS', src: '/logos/tcs.jpeg' },
  { name: 'Tech Mahindra', src: '/logos/tech-mahindra.png' },
  { name: 'HCLTech', src: '/logos/hcltech.png' },
  { name: 'Infosys', src: '/logos/infosys.png' },
  { name: 'NVIDIA', src: '/logos/nvidia.png' },
  { name: 'Amex', src: '/logos/amex.png' },
  // { name: 'Google', src: '/logos/google.jpeg' },
];
  ...
  return (
    <section className="bg-surface-alt py-10 sm:py-12">
      <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
        ...
        {/* Desktop */}
        <div className="hidden flex-wrap items-center justify-center gap-x-10 gap-y-6 md:flex">
          {COMPANY_LOGOS.map((company) => (
            <div
              key={company.name}
            ...
            >
              <img
                ...
                className="max-h-10 max-w-[140px] object-contain opacity-100  transition-all duration-300 hover:opacity-50 hover:grayscale-0"
                loading="lazy"
              />
            </div>
          ))}
          ...
          {COMPANY_LOGOS.map((company) => (
            ...
            >
              <img
                src={company.src}
                alt={`${company.name} logo`}
                className="max-h-10 max-w-full object-contain opacity-100  transition-all duration-300 hover:opacity-50 hover:grayscale-0"
                loading="lazy"
              />
            </div>
          ))}
